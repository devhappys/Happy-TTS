...
export class StorageManager {
  ...
  private static ensureRecordsFile() {
    const dir = path.dirname(StorageManager.RECORDS_FILE);
    if (!fs.existsSync(dir)) {
      fs.mkdirSync(dir, { recursive: true });
    }
    if (!fs.existsSync(StorageManager.RECORDS_FILE)) {
      fs.writeFileSync(StorageManager.RECORDS_FILE, JSON.stringify([]));
    }
  }

  private static readRecords(): GenerationRecord[] {
    StorageManager.ensureRecordsFile();
    try {
      const data = fs.readFileSync(StorageManager.RECORDS_FILE, "utf-8");
  ...
  private static writeRecords(records: GenerationRecord[]) {
    try {
      fs.writeFileSync(StorageManager.RECORDS_FILE, JSON.stringify(records, null, 2));
    } catch (error) {
  ...
  public static async addRecord(ip: string, fingerprint: string, text: string, fileName: string): Promise<void> {
    ...
    // 如果记录数超过最大值，删除最旧的记录
    if (records.length > StorageManager.MAX_RECORDS) {
      records.sort((a, b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime());
      records.splice(StorageManager.MAX_RECORDS);
    }
  ...
  public static async checkDuplicate(ip: string, fingerprint: string, text: string): Promise<boolean> {
    ...
    return records.some((record) => {
      ...
      return (
        recordTime > windowStart && record.text === text && (record.ip === ip || record.fingerprint === fingerprint)
      );
    });
  }
