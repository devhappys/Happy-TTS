...
import {
  authLimiter, meEndpointLimiter, ttsLimiter, historyLimiter,
  adminLimiter, frontendLimiter, totpLimiter, passkeyLimiter,
  tamperLimiter, commandLimiter, libreChatLimiter, dataCollectionLimiter,
  logsLimiter, ipfsLimiter, networkLimiter, dataProcessLimiter,
  mediaLimiter, socialLimiter, lifeLimiter, miniapiLimiter,
  antaLimiter, statusLimiter, openapiLimiter, audioFileLimiter,
  modlistMountLimiter, cdkMountLimiter, githubBillingLimiter,
  integrityLimiter, rootLimiter, lcCompatLimiter, ipQueryLimiter,
  ipLocationLimiter, ipReportLimiter, serverStatusLimiter,
  staticFileLimiter, docsTimeoutLimiter, globalDefaultLimiter,
  notFoundLimiter,
...
app.use(helmet({
  contentSecurityPolicy: {
    directives: {
      ...
      connectSrc: [
        "'self'",
        "https://api.openai.com", "https://api.hapxs.com",
        "http://localhost:3000", "http://localhost:3001",
        "http://localhost:6000", "http://localhost:6001",
        "http://127.0.0.1:3001", "http://127.0.0.1:6000", "http://127.0.0.1:6001",
        "http://192.168.10.7:3001", "http://192.168.10.7:6000", "http://192.168.10.7:6001",
...
app.use('/api-docs', (req: Request, res: Response, next: NextFunction) => {
  ...
  ? swaggerUi.setup(undefined, { swaggerUrl: '/openapi.json', customSiteTitle: 'Happy API', customCss: swaggerCustomCss })
...
app.post('/api/report-docs-timeout', docsTimeoutLimiter, express.json(), (req, res) => {
  ...
  logger.error('API文档加载超时', {
    url, timestamp: new Date(timestamp).toISOString(), userAgent,
...
// 禁止二次校验相关接口缓存
app.use([
  '/api/totp/status', '/api/passkey/credentials',
  '/api/passkey/authenticate/start', '/api/passkey/authenticate/finish',
  '/api/passkey/register/start', '/api/passkey/register/finish',
  '/api/auth/me', '/api/auth/logout', '/api/auth/login', '/api/auth/register',
