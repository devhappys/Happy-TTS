...
export class NexaiSyncController {
  ...
  static async putSyncData(req: Request, res: Response) {
    try {
      const userId = req.nexaiUser?.id;
      const { settings, notes, conversations, translationHistory, savedPasswords, shortUrls } = req.body;
  ...
  static async incrementalSync(req: Request, res: Response) {
    try {
      ...
      const serverChanges = await NexaiSyncService.mergeIncrementalData(userId, data, lastSyncedAt);
