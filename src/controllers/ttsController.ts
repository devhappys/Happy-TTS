...
import { addGenerationRecord, findDuplicateGeneration, isAdminUser } from "../services/userGenerationService";
...
export class TtsController {
  ...
  public static async generateSpeech(req: Request, res: Response) {
    try {
      ...
      if (userId && !isAdmin) {
        // 生成内容哈希
        const contentHash = require("../services/ttsService").TtsService.prototype.generateContentHash(
          text,
          voice,
          model,
        );
        ...
        if (duplicate?.fileName) {
          return res.json({
            ...
            audioUrl: `${process.env.VITE_API_URL || process.env.BASE_URL || "https://api.951100.xyz"}/static/audio/${duplicate.fileName}`,
