...
import rateLimit, { type RateLimitRequestHandler, type Store, type IncrementResponse, type Options } from 'express-rate-limit';
...
export const meEndpointLimiter = createLimiter({ name: 'me', windowMs: 5 * 60_000, max: 300, message: '请求过于频繁，请稍后再试' });
...
// --- 二次验证 ---
export const totpLimiter = createLimiter({ name: 'totp', windowMs: 5 * 60_000, max: 20, message: 'TOTP操作过于频繁，请稍后再试' });
export const passkeyLimiter = createLimiter({ name: 'passkey', windowMs: 5 * 60_000, max: 30, message: 'Passkey操作过于频繁，请稍后再试' });
...
// --- LibreChat ---
export const libreChatLimiter = createLimiter({ name: 'librechat', max: 60, message: 'LibreChat请求过于频繁，请稍后再试' });

// --- 数据收集 ---
export const dataCollectionLimiter = createLimiter({ name: 'datacollection', max: 30, message: '数据收集请求过于频繁，请稍后再试' });
...
// --- 数据处理 ---
export const dataProcessLimiter = createLimiter({ name: 'dataprocess', max: 50, message: '数据处理请求过于频繁，请稍后再试' });
...
// --- MOD 列表 ---
export const modlistMountLimiter = createLimiter({ name: 'modlist', max: 60, message: 'MOD列表请求过于频繁，请稍后再试' });

// --- CDK ---
export const cdkMountLimiter = createLimiter({ name: 'cdk', max: 60, message: 'CDK 请求过于频繁，请稍后再试' });

// --- GitHub Billing ---
export const githubBillingLimiter = createLimiter({ name: 'ghbilling', max: 10, message: 'GitHub Billing请求过于频繁，请稍后再试' });
...
export const ipLocationLimiter = createLimiter({ name: 'iplocation', max: 20, message: 'IP位置查询过于频繁，请稍后再试' });
...
// --- 服务器状态 ---
export const serverStatusLimiter = createLimiter({ name: 'serverstatus', max: 10, message: '状态查询过于频繁，请稍后再试' });

// --- 静态文件 ---
export const staticFileLimiter = createLimiter({ name: 'static', max: 200, message: '静态文件请求过于频繁，请稍后再试' });
