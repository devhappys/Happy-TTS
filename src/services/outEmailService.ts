    ...
    const batch = messages.map((m) => {
      ...
      return {
        ...
        ...(sender.name && sender.name !== fromUser
          ? { reply_to: `${sender.name} <${sender.email}>`, headers: { "X-From-Name": sender.name } }
          : {}),
      } as any;
    });
...
export async function sendOutEmail({
  ...
  displayName?: string;
  domain?: string;
  attachments?: Array<{ path?: string; content?: string; filename: string; content_id?: string }>;
}) {
  // 检查对外邮件服务状态
  const outemailStatus = (globalThis as any).OUTEMAIL_SERVICE_STATUS;
  if (outemailStatus && !outemailStatus.available) {
  ...
  try {
    ...
    if (attachments && Array.isArray(attachments) && attachments.length) {
      ...
      const safeAttachments = attachments
        .filter(
          (a) => a && typeof a.filename === "string" && (typeof a.path === "string" || typeof a.content === "string"),
        )
        .slice(0, 10)
        .map((a) => ({
          filename: a.filename,
