...
const DebugConsoleConfigModel =
  (mongoose.models.DebugConsoleConfig as any) || mongoose.model("DebugConsoleConfig", DebugConsoleConfigSchema);
...
const DebugConsoleAccessLogModel =
  (mongoose.models.DebugConsoleAccessLog as any) ||
...
class DebugConsoleService {
  ...
  private async getConfigsFresh(): Promise<DebugConsoleConfigDoc[]> {
    ...
    if (!this.configLoadedAt || now - this.configLoadedAt > this.CONFIG_TTL_MS || (isConnected && !wasConnected)) {
      await this.loadConfigs();
    }
  ...
  public async getAccessLogs(
    ...
    try {
      ...
      const logs = await DebugConsoleAccessLogModel.find(query)
        .sort({ timestamp: -1 })
