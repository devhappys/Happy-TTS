...
export class RecommendationService {
   ...
   * @returns 推荐列表
   */
  async getPersonalizedRecommendations(userId: string, limit: number = DEFAULT_RECOMMENDATION_LIMIT): Promise<Recommendation[]> {
    try {
      // 获取用户历史记录
      ...
      // 分析用户历史，生成个性化推荐
      const recommendations = this.analyzeHistoryForRecommendations(
        history.generations,
        disabledCategories,
        limit
      );
  ...
  private analyzeHistoryForRecommendations(
    ...
    // 按频率排序
    const sortedStyles = Array.from(styleFrequency.values())
      .sort((a, b) => b.count - a.count);

    // 过滤禁用的类别
    const filteredStyles = sortedStyles.filter(
      item => !disabledCategories.includes(item.style.emotionalTone)
    );

    // 生成推荐
    const recommendations: Recommendation[] = filteredStyles
      .slice(0, limit)
      .map((item, index) => ({
    ...
    if (recommendations.length < limit) {
      ...
      const additionalStyles = DEFAULT_POPULAR_STYLES
        .filter(s => !existingIds.has(s.id) && !disabledCategories.includes(s.emotionalTone))
        .slice(0, remaining);
