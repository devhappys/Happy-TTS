...
describe("Rate Limiting", () => {
  ...
  it("应该允许正常请求", async () => {
    const response = await request(app).get("/test").set("X-Forwarded-For", "192.168.1.1").expect(200);
  ...
  it("应该在超过限制时返回429错误", async () => {
    ...
    // 下一个请求应该被限制
    const response = await request(app).get("/test").set("X-Forwarded-For", "192.168.1.1").expect(429);
  ...
  it("应该为不同IP分别计数", async () => {
    ...
    const response = await request(app).get("/test").set("X-Forwarded-For", "192.168.1.2").expect(200);
