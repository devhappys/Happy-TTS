...
describe("LibreChat Routes", () => {
  ...
  describe("POST /api/libre-chat/send", () => {
    it("应该拒绝无效token的请求", async () => {
      const res = await request(app).post("/api/libre-chat/send").send({
        token: "invalid-token",
  ...
  describe("DELETE /api/libre-chat/clear", () => {
    it("应该成功清除聊天历史", async () => {
      const res = await request(app).delete("/api/libre-chat/clear").send({ token: validToken });
    ...
    it("应该拒绝无效token的清除请求", async () => {
      const res = await request(app).delete("/api/libre-chat/clear").send({ token: "invalid-token" });
