...
describe("精准IP查询 API", () => {
  ...
  testCases.forEach((tc) => {
    it(`${tc.name} (${tc.ip}) 查询应成功`, async () => {
      const res = await request(app).get("/api/network/ipquery").query({ ip: tc.ip });
  ...
  it("空IP参数应返回400", async () => {
    const res = await request(app).get("/api/network/ipquery");

    expect(res.status).toBe(400);
  });

  it("无效IP格式应返回400", async () => {
    const res = await request(app).get("/api/network/ipquery").query({ ip: "invalid-ip" });
