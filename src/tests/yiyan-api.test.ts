...
async function expectYiyanOk(type: "hitokoto" | "poetry") {
  const res = await request(app).get("/api/network/yiyan").query({ type });
...
describe("随机一言古诗词 API", () => {
  ...
  it("空类型参数应该返回 400 错误", async () => {
    const res = await request(app).get("/api/network/yiyan");

    expect(res.status).toBe(400);
  });

  it("无效类型参数应该返回 400 错误", async () => {
    const res = await request(app).get("/api/network/yiyan").query({ type: "invalid" });
