...
describe("IPFSService", () => {
  describe("uploadFile", () => {
    it("应该验证文件大小限制", async () => {
      ...
      await expect(IPFSService.uploadFile(largeBuffer, "test.jpg", "image/jpeg")).rejects.toThrow(
        "文件大小不能超过 5MB",
      );
    });
    ...
    it("应该验证文件类型", async () => {
      ...
      await expect(IPFSService.uploadFile(buffer, "test.txt", "text/plain")).rejects.toThrow(
