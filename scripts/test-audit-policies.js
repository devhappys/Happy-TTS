 ...
 *
 * Run: node scripts/test-audit-policies.js
 */
...
test("weak root:password URI is rejected", () => {
  assert.strictEqual(isWeakMysqlUri("mysql://root:password@localhost:3306/tts"), true);
  assert.throws(
    () => requireMysqlUri({ MYSQL_URI: "mysql://root:password@localhost:3306/tts" }),
    /weak|default/i,
  );
});
