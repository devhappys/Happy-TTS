...
def upload_log_file(log_path, admin_password):
    """
    上传日志/文件到 https://api.951100.xyz/api/sharelog，返回短链。
    ...
    if file_size >= 25600:
        ...
        return None
    url = "https://api.951100.xyz/api/sharelog"
        ...
        try:
            ...
            if resp.ok:
                ...
                if not link and "id" in j:
                    # 兼容只返回id的情况
                    link = f"https://api.951100.xyz/logshare?id={j['id']}"
                if link:
...
def query_log_file(log_id, admin_password):
    """
    查询日志内容，POST到 https://api.951100.xyz/api/sharelog/{id}，body为{adminPassword}
    """
    url = f"https://api.951100.xyz/api/sharelog/{log_id}"
    data = {"adminPassword": admin_password}
    try:
