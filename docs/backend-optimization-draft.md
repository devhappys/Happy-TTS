...
这份草案用于指导 Synapse 后端下一阶段优化工作，目标不是推翻现有实现，而是在当前代码基础上做结构收口、可靠性增强和运行治理补齐。
...
当前 Synapse 后端已经完成了第一轮重要重构，旧的”先拆入口、先拆 UserStorage”的思路已经不再是主战场。当前最值得投入的方向是：
...
如果这四件事落地，Synapse 后端的维护成本、可靠性和后续扩展能力都会明显改善。这也是最符合当前代码现状的优化路线。
