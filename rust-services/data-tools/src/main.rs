...
async fn main() {
    ...
    let config = match DataToolsConfig::from_env() {
        Ok(config) => config,
        Err(error) => {
            tracing::error!(error = %error, "failed to load data-tools config");
            std::process::exit(1);
        }
    };
    ...
    let listener = match tokio::net::TcpListener::bind(bind_addr).await {
        ...
        Err(error) => {
            tracing::error!(bind_addr = %bind_addr, error = %error, "failed to bind data-tools");
            std::process::exit(1);
        }
    };

    tracing::info!(bind_addr = %bind_addr, "data-tools listening");
