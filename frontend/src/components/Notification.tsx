            ...
            <AnimatePresence>
                {notification && (
                    <motion.div
                        ...
                        className={`fixed top-4 right-4 z-[9999] ${getTypeStyles(notification.type)} text-white px-3 sm:px-4 py-2 sm:py-3 rounded-lg shadow-lg backdrop-blur-sm bg-opacity-95 flex items-center min-w-[180px] max-w-xs`}
                        style={{ gap: 8 }}
                    >
                        <div className="text-sm sm:text-base font-medium break-all pr-2 flex-1">{notification.message}</div>
                        <button
                            onClick={handleClose}
                            className="p-1 sm:p-2 rounded-full hover:bg-white/20 focus:outline-none transition-all duration-150 flex items-center justify-center"
                            aria-label="关闭通知"
