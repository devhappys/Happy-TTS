...
interface SelfContainedHcaptchaConfigSectionProps {
...
export default function SelfContainedHcaptchaConfigSection({ prefersReducedMotion: reducedMotionProp }: SelfContainedHcaptchaConfigSectionProps) {
  ...
  const [config, setConfig] = useState<HCaptchaConfigSetting | null>(null);
  ...
  const fetchConfig = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        else setNotification({ message: data.error || '获取hCaptcha配置失败', type: 'error' });
        return;
      }
  ...
  const handleSave = useCallback(async (key: 'HCAPTCHA_SECRET_KEY' | 'HCAPTCHA_SITE_KEY') => {
    ...
    setSaving(true);
    try {
      const res = await authFetch(HCAPTCHA_CONFIG_API, { method: 'POST', headers: { 'Content-Type': 'application/json', ...getAuthHeaders() }, body: JSON.stringify({ key, value }) });
      const data = await res.json();
      ...
      if (key === 'HCAPTCHA_SECRET_KEY') setSecretKeyInput(''); else setSiteKeyInput('');
      await fetchConfig();
