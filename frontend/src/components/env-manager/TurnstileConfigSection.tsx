...
export default function TurnstileConfigSection({
  ...
  return (
    <CollapsibleSection title="Turnstile 配置设置" description="管理 Cloudflare Turnstile Site Key 和 Secret Key。" sectionKey="turnstile" isOpen={isOpen} onToggle={onToggle} prefersReducedMotion={prefersReducedMotion} headerRight={
              ...
              <div className="mb-6">
                ...
                <div className="flex items-center justify-end gap-3">
                  <m.button
                    onClick={() => onDelete('TURNSTILE_SITE_KEY')}
                    disabled={deleting}
                  ...
                  <m.button
                    onClick={() => onSave('TURNSTILE_SITE_KEY')}
                    disabled={saving}
              ...
              <div className="mb-4">
                ...
                <div className="flex items-center justify-end gap-3">
                  <m.button
                    onClick={() => onDelete('TURNSTILE_SECRET_KEY')}
                    disabled={deleting}
                    className="px-3 sm:px-4 py-2 bg-red-500 text-white rounded-lg hover:bg-red-600 transition disabled:opacity-50 text-sm font-medium"
                    whileTap={{ scale: 0.96 }}
                  >
                  ...
                  <m.button
                    onClick={() => onSave('TURNSTILE_SECRET_KEY')}
                    disabled={saving}
                    className="px-3 sm:px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition disabled:opacity-50 text-sm font-medium"
                    whileTap={{ scale: 0.96 }}
                  >
