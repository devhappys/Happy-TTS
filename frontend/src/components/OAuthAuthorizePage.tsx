...
const getErrorMessage = (error: any) => {
  ...
  return data?.error_description || data?.error || error?.message || '授权请求无法处理';
};
...
const OAuthAuthorizePage: React.FC = () => {
  ...
  const loginPath = useMemo(() => buildLoginPath(location.pathname, location.search), [location.pathname, location.search]);

  const requestPayload = useCallback((approve: boolean) => {
    ...
    return (
      <div className="mx-auto flex min-h-[60vh] max-w-3xl items-center justify-center px-4 py-10">
        <div className="w-full rounded-2xl border border-slate-200 bg-white/90 p-8 text-center shadow-sm">
          ...
          <p className="mt-4 text-sm text-slate-500">正在读取 OAuth 授权请求...</p>
        </div>
      </div>
    );
  }
        ...
        <div className="rounded-2xl border border-rose-100 bg-white p-6 shadow-sm sm:p-8">
          <div className="flex items-start gap-4">
            ...
            <div className="min-w-0 flex-1">
              <h1 className="text-2xl font-semibold text-slate-900">OAuth 授权不可用</h1>
              <p className="mt-3 text-sm leading-7 text-slate-600">{error || '授权请求无效'}</p>
        ...
        <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-7">
            ...
            <div className="flex min-w-0 items-start gap-4">
              <div className="flex h-14 w-14 shrink-0 items-center justify-center overflow-hidden rounded-xl border border-slate-200 bg-slate-50 text-slate-500">
                {preview.client.logoUrl ? (
                  <img src={preview.client.logoUrl} alt="" className="h-full w-full object-cover" />
                ) : (
                  <FaKey />
                )}
              </div>
              <div className="min-w-0">
                <div className="text-xs font-semibold uppercase tracking-[0.22em] text-slate-400">OAuth Authorization</div>
                <h1 className="mt-2 break-words text-2xl font-semibold text-slate-900 sm:text-3xl">
                  {preview.client.name}
                </h1>
                {preview.client.description && (
                  <p className="mt-2 text-sm leading-7 text-slate-600">{preview.client.description}</p>
                )}
              </div>
            </div>
          ...
          <div className="mt-6 grid gap-3 text-sm">
            <div className="rounded-xl border border-slate-200 bg-slate-50 p-4">
              <div className="text-xs font-semibold text-slate-400">回调地址</div>
          ...
          <div className="mt-6">
            ...
            <div className="mt-3 grid gap-3 sm:grid-cols-2">
              {identityScopes.map((scope) => (
                <div key={scope.key} className="rounded-xl border border-slate-200 bg-white p-4">
                  <div className="flex items-center justify-between gap-3">
                    <span className="font-semibold text-slate-800">{scope.label}</span>
                    <code className="rounded bg-slate-100 px-2 py-1 text-xs text-slate-600">{scope.key}</code>
                  </div>
                  <p className="mt-2 text-xs leading-6 text-slate-500">{scope.description}</p>
                </div>
              ))}
            </div>
          </div>
                    ...
                    {scope.endpoints.length > 0 && (
                      <div className="mt-3 flex flex-wrap gap-2">
                        {scope.endpoints.map((endpoint) => (
                          <code key={endpoint} className="rounded bg-slate-50 px-2 py-1 text-xs text-slate-500">
                            {endpoint}
                          </code>
                        ))}
                      </div>
          ...
          <div className="mt-7 flex flex-col gap-3 sm:flex-row">
            <motion.button
              ...
              whileTap={{ scale: 0.98 }}
            >
              {submitting === 'approve' ? <SimpleLoadingSpinner size={0.5} /> : <FaCheck />}
              同意授权
            </motion.button>
            <motion.button
              type="button"
              ...
              whileTap={{ scale: 0.98 }}
            >
              {submitting === 'deny' ? <SimpleLoadingSpinner size={0.5} /> : <FaTimes />}
              拒绝
            </motion.button>
          </div>
        </section>
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            ...
            <div className="mt-4 space-y-3 text-sm text-slate-600">
              <div className="rounded-xl bg-slate-50 p-3">
                <div className="text-xs font-semibold text-slate-400">授权码模式</div>
                <code className="mt-2 block break-all text-xs text-slate-700">
                  response_type=code
                </code>
              </div>
              <div className="rounded-xl bg-slate-50 p-3">
                <div className="text-xs font-semibold text-slate-400">授权范围</div>
                <code className="mt-2 block break-all text-xs text-slate-700">{preview.scopes.join(' ')}</code>
              </div>
              <div className="rounded-xl bg-slate-50 p-3">
